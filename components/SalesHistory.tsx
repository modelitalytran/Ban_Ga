...
import { Order, CartItem, ImportRecord } from '../types';
import { Calendar, User, Tag, AlertCircle, CheckCircle, Edit2, X, Save, Scale, Info, ShoppingCart, ArrowDownCircle, FileText } from 'lucide-react';

interface SalesHistoryProps {
  orders: Order[];
  imports: ImportRecord[]; // New Prop
  onEditOrder?: (updatedOrder: Order, originalOrder: Order) => void;
}

const SalesHistory: React.FC<SalesHistoryProps> = ({ orders, imports, onEditOrder }) => {
  const [activeTab, setActiveTab] = useState<'sales' | 'imports'>('sales');

  // Sort orders by date descending
  const sortedOrders = [...orders].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  
  // Sort imports
  const sortedImports = [...imports].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex p-1 bg-gray-100 rounded-xl">
             ...
             >
                 <ShoppingCart size={16}/> Lịch sử Bán
             </button>
             <button
                onClick={() => setActiveTab('imports')}
                className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                    activeTab === 'imports' ? 'bg-white text-teal-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'
                }`}
             >
                 <ArrowDownCircle size={16}/> Lịch sử Nhập
             </button>
        </div>
      </div>
            ...
            <div className="overflow-x-auto">
                ...
                <tbody className="divide-y divide-gray-100">
                    ...
                    {sortedOrders.length === 0 && (
                        <tr><td colSpan={6} className="p-8 text-center text-gray-500">Chưa có lịch sử bán hàng.</td></tr>
                    )}
                </tbody>
                </table>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div className="overflow-x-auto">
                ...
                <thead className="bg-teal-50 border-b border-teal-100">
                    <tr>
                    <th className="p-4 font-semibold text-teal-800">Ngày Nhập</th>
                    <th className="p-4 font-semibold text-teal-800">Sản phẩm</th>
                    <th className="p-4 font-semibold text-teal-800">Số lượng nhập</th>
                    <th className="p-4 font-semibold text-teal-800">Chi phí nhập</th>
                    <th className="p-4 font-semibold text-teal-800">Nguồn gốc / Ghi chú</th>
                    </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                    {sortedImports.map(record => (
                        <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                            <td className="p-4">
                                <div className="flex flex-col">
                                    <span className="font-medium text-gray-900 text-sm">{new Date(record.date).toLocaleDateString('vi-VN')}</span>
                                    <span className="text-xs text-gray-500">{new Date(record.date).toLocaleTimeString('vi-VN', {hour: '2-digit', minute:'2-digit'})}</span>
                                </div>
                            </td>
                            <td className="p-4">
                                <div className="flex items-center gap-3">
                                    <img src={record.productImage} className="w-10 h-10 rounded-lg object-cover bg-gray-100 border border-gray-200"/>
                                    <span className="font-bold text-gray-800">{record.productName}</span>
                                </div>
                            </td>
                            <td className="p-4">
                                <span className="inline-flex items-center gap-1 px-3 py-1 bg-teal-100 text-teal-800 rounded-full font-bold text-sm">
                                    <ArrowDownCircle size={14}/> +{record.quantity}
                                </span>
                            </td>
                            <td className="p-4">
                                {record.importPrice ? (
                                    <span className="font-medium text-gray-700">{record.importPrice.toLocaleString('vi-VN')} ₫</span>
                                ) : (
                                    <span className="text-xs text-gray-400 italic">Không ghi nhận</span>
                                )}
                            </td>
                    ...
                    {sortedImports.length === 0 && (
                        <tr><td colSpan={5} className="p-8 text-center text-gray-500">Chưa có lịch sử nhập hàng.</td></tr>
                    )}
                </tbody>
                </table>
            </div>
